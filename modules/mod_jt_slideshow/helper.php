...

{
...

    {
		...
		$this->gallerythumbnailwidth	= trim ( $params->get ( 'gallerythumbnailwidth', '125px' ) );

		$this->gallerythumbnailheight	= trim ( $params->get ( 'gallerythumbnailheight', '100%' ) );

		

		$this->gallerythumbnailspace		= trim ( $params->get ( 'gallerythumbnailspace', '2px' ) );
		...
		$this->gallerythumbnailactivebordercolor 	= trim ( $params->get ( 'gallerythumbnailactivebordercolor', '#88f' ) );
		...
		if ($this->imgborder == 1) $css = $css . ' DIV#'.$this->boxname.' img {  } ';

		// Caption border-width: '. $this->imgbordersize .'; border-style: solid; border-color: '. $this->imgbordercolor .';
		...
		$css = $css . ' #'.$this->boxname.'nav { display:none; margin-top:5px; margin-bottom:5px; float:right; } ';
		...
		if ($this->paginationitemsborder == 1) $css = $css . ' #'.$this->boxname.'nav a {background: url("selected_bg.png") repeat scroll 0 0 transparent; color: #000000;float: left;font-size: 11px;font-size: 11px; font-weight: bold; font-weight: bold; height: 13px; line-height: 1.5em; margin: 5px;margin: 2px;padding: 5px;padding: 2px;text-align: center; text-decoration: none; width: 14px;} ';

		$css = $css . ' #'.$this->boxname.'nav a:focus { outline: none; } ';

		// Pagination Bar - Active Links

		$css = $css . ' #'.$this->boxname.'nav a.activeSlide { color: #000; background: url("hover_bg.png") no-repeat scroll 0 0 transparent; height:17px; } ';
		...
		$css = $css . ' #'.$this->boxname.'gallery { width: '.$this->gallerywidth.'; list-style-type: none; margin: 0; padding: 0; '.$this->galleryposition. ' } ';

		// Gallery Items

		$css = $css . ' #'.$this->boxname.'gallery li { float:'.$this->gallerydirection.'; margin:'.$this->gallerythumbnailspace.'; }';

		// Gallery Links

		$css = $css . ' #'.$this->boxname.'gallery a { width: '.$this->gallerythumbnailwidth.'; height: '.$this->gallerythumbnailheight.'; display: block; }';

		if ($this->gallerythumbnailborder == 1) $css = $css . ' #'.$this->boxname.'gallery a { border-width: '. $this->gallerythumbnailbordersize .'; border-style: solid; border-color: '. $this->gallerythumbnailbordercolor .'; }';

		// Gallery Active Links

		$css = $css . ' #'.$this->boxname.'gallery a.activeSlide { border-width: '. $this->gallerythumbnailbordersize .'; border-style: solid; border-color: '. $this->gallerythumbnailactivebordercolor .'; }';
